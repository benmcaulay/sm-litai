...
-- Policies
create policy if not exists "Users can view databases in their firm"
  on public.external_databases for select
  using (firm_id = get_user_firm_id());

create policy if not exists "Users can create databases in their firm"
  on public.external_databases for insert
  with check (firm_id = get_user_firm_id() and created_by = auth.uid());

create policy if not exists "Users can update their own databases"
  on public.external_databases for update
  using (created_by = auth.uid());

create policy if not exists "Admins can update all firm databases"
  on public.external_databases for update
  using (is_admin() and firm_id = get_user_firm_id());

-- Trigger to update updated_at
create trigger if not exists update_external_databases_updated_at
