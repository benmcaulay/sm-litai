...
serve(async (req) => {
  ...
  try {
    ...
    console.log('NetDocs folder contents request for folder:', folderId, 'user:', uid);
    ...
    // Get folder contents
    const folderUrl = `${ndBaseUrl}/v1/Folder/${folderId}/contents`;
    const folderResponse = await fetch(folderUrl, { 
      headers: { Authorization: `Bearer ${accessToken}` } 
    });
    
    if (!folderResponse.ok) {
      console.error('Failed to fetch folder contents:', folderResponse.status, await folderResponse.text());
      return new Response(JSON.stringify({ error: "Failed to fetch folder contents" }), { 
    ...
    // Normalize the response to match user's requested format
    const items = (folderData.items || []).map((item: any) => ({
      id: item.id,
      name: item.name || item.displayName,
      type: item.type || (item.extension ? 'document' : 'folder'),
      ext: item.extension || '',
      size: item.size || 0,
    ...
    console.log('Retrieved folder items:', items.length);
    ...
    console.error('Error in netdocs-folder-contents:', error);
